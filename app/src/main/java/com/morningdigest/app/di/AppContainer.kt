...
class AppContainer(context: Context) {
    ...
    val metWeatherFetcher: MetWeatherFetcher by lazy { MetWeatherFetcher(okHttpClient) }

    val policeIncidentFetcher: PoliceIncidentFetcher by lazy { PoliceIncidentFetcher(okHttpClient) }

    private val database: AppDatabase by lazy { AppDatabase.getInstance(context) }

    val digestRepository: DigestRepository by lazy {
        DigestRepository.create(database, openWeatherApi, coinGeckoApi, frankfurterApi, rssFeedFetcher, stockMoversFetcher, youtubeChannelFetcher, liveCurrencyFetcher, metWeatherFetcher)
    }
