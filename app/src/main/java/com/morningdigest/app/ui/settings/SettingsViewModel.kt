...
class SettingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun updatePoliceSettings(municipality: String, enabled: Boolean, categories: Set<String>) = viewModelScope.launch {
        container.settingsRepository.updatePoliceSettings(municipality, enabled, categories)
        WorkScheduler.applySchedule(getApplication(), _uiState.value.settings.copy(
            policeMunicipality = municipality, policeAlertsEnabled = enabled, policeCategories = categories
        ))
