...
data class DashboardUiState(
    ...
    val policeIncidents: List<PoliceIncidentFetcher.Incident> = emptyList(),
...
class DashboardViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun refreshPolice() {
        viewModelScope.launch {
            ...
            if (!settings.policeAlertsEnabled) {
                _uiState.value = _uiState.value.copy(policeIncidents = emptyList())
                return@launch
            }
            _uiState.value = _uiState.value.copy(isRefreshingPolice = true)
            runCatching {
                container.policeIncidentFetcher.fetch(settings.policeMunicipality, settings.policeCategories)
            }.onSuccess { incidents ->
                _uiState.value = _uiState.value.copy(policeIncidents = incidents, isRefreshingPolice = false)
            }.onFailure {
                _uiState.value = _uiState.value.copy(isRefreshingPolice = false)
            }
        }
    }
    ...
    fun refreshNow() {
        viewModelScope.launch {
            ...
            try {
                val settings = container.settingsRepository.currentSettings()
                val saved = container.digestRepository.refreshDashboardSection(settings)
                _uiState.value = _uiState.value.copy(
                    report = saved,
                    isLoading = false,
    ...
    fun dismissYoutubeVideo(channelId: String, videoId: String) {
        ...
        viewModelScope.launch {
            ...
            val updatedChannels = settings.youtubeChannels.map { channel ->
                if (channel.channelId == channelId) channel.copy(dismissedVideoIds = channel.dismissedVideoIds + videoId)
                else channel
            }
    ...
    fun dismissAllYoutubeVideos() {
        ...
        viewModelScope.launch {
            ...
            val updatedChannels = settings.youtubeChannels.map { channel ->
                ...
                else channel.copy(dismissedVideoIds = channel.dismissedVideoIds + newlyDismissed)
            }
            container.settingsRepository.updateYoutubeChannels(updatedChannels)
        }
    }
